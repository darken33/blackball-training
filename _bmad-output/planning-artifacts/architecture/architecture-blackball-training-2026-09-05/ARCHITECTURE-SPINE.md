...
### AD-2 — Les schémas sont un rendu, jamais un asset dessiné [SUPERSEDÉ — cf. spec-schemas-officiels.md]

- **Binds:** FR-1, FR-2
- **Prevents:** un moteur de score qui déduirait le nombre de billes de la séquence depuis la longueur de `ordrePoche` — faux pour les exercices "lecture de table" (Or A/C/D/F) où `ordrePoche` est vide alors que la séquence compte 3 à 5 billes. *(Clause toujours en vigueur : `nombreBillesSequence` reste un champ Exercice indépendant, jamais dérivé.)*
- **Rule:** le catalogue distingue **Figure** (schéma partagé) et **Exercice** (bille/poche ciblée, `ordrePoche: number[] | null`, `nombreBillesSequence: number` — toujours renseigné). En Bronze une Figure porte 3-4 Exercices partageant le même schéma ; en Argent/Or une Figure = un seul Exercice (glossaire PRD §3). *(Toujours en vigueur — cette distinction n'a pas changé.)*
- **Historique de la décision :** la justification initiale de "rendu programmatique, jamais un asset dessiné" — éviter 35 images redessinées à la main avec un risque de divergence de fidélité entre exercices — est retirée depuis `spec-schemas-officiels.md` (2026-09-07) : le rendu SVG généré depuis les coordonnées de grille ne respectait pas assez fidèlement les schémas officiels du règlement (constat utilisateur). Le composant `table-diagram` affiche désormais directement l'image officielle correspondant à la Figure (une image par Figure — pas par Exercice, la distinction Figure/Exercice ci-dessus reste inchangée — 21 assets, source officielle unique, pas redessinés à la main). Le risque de divergence de fidélité que cette clause voulait éviter est mieux couvert par des images officielles que par une approximation vectorielle.

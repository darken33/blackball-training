...
- **Table diagram** — image officielle du schéma (une par Figure, `gfx/base/*.png` → `src/assets/schemas/`), affichée telle quelle dans un cadre `table-rail` arrondi. Remplace un précédent rendu vectoriel généré depuis les coordonnées de grille (moins fidèle aux schémas du règlement, cf `spec-schemas-officiels.md`) ; la distinction Figure/Exercice (une Figure Bronze partagée par plusieurs Exercices, cf PRD FR-2) reste la même, seule la technique de rendu change.
